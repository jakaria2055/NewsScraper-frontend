import React from "react";
import NewsStore from "../../store/NewsStore";
import NewsSkeleton from "../../skeleton/NewsSkeleton";
import { Link } from "react-router-dom";

function UserNewsList() {
  const { NewsList, RemoveNewsRequest, NewsListRequest } = NewsStore();

  const remove = async (newsId) => {
    await RemoveNewsRequest(newsId);
    await NewsListRequest();
  };

   console.log(NewsList);

  if (NewsList === null) {
    return <NewsSkeleton />;
  } else {
    return (
      <div className="px-10 space-y-2 mt-2">
        {NewsList.map((item, i) => {
          return (
            <div key={i}>
              <ul className="list bg-base-100 rounded-box shadow-md">
                <li className="list-row">
                  <div>
                    <img className="size-10 rounded-box" src={item["image"]} />
                  </div>
                  <div>
                    <div className="text-xs uppercase font-semibold opacity-60">
                      {item["title"]}
                    </div>
                    <div className="p-4 pb-2 text-xs opacity-60 tracking-wide">
                      {" "}
                      {item["category"]}
                    </div>
                  </div>
                  <Link
                    to={`/edit/${item["_id"]}`}
                    className="btn btn-square btn-ghost"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                    >
                      <path
                        fill="currentColor"
                        d="M5.38 6.808H18.6l-1.33-1.596q-.097-.096-.222-.154T16.788 5H7.192q-.134 0-.26.058t-.22.154zM10 12.962l2-1l2 1V7.808h-4zM5.616 20q-.667 0-1.141-.475T4 18.386v-10.9q0-.282.093-.545t.28-.48l1.558-1.87q.217-.286.543-.438T7.176 4h9.61q.376 0 .709.153q.333.152.555.438L19.627 6.5q.187.225.28.492q.093.266.093.552v2.023q0 .213-.144.356t-.357.144t-.356-.144T19 9.567v-1.76h-4v5.857l-.617.617L12 13.096l-1.839.91q-.403.211-.782-.023Q9 13.748 9 13.279V7.808H5v10.577q0 .269.173.442t.443.173h5.73q.213 0 .357.144t.143.357t-.143.356t-.357.143zM15 7.808h4zm-10 0h9.383zm9.23 11.384V18.12q0-.153.057-.296q.055-.144.186-.275l5.09-5.065q.149-.148.306-.2q.158-.052.315-.052q.172 0 .338.064q.166.065.301.194l.925.944q.123.148.188.308q.064.159.064.319t-.061.322t-.191.31l-5.065 5.066q-.131.13-.275.186q-.143.056-.297.056h-1.073q-.343 0-.575-.232t-.232-.576m6.884-5.133l-.925-.944zm-6 5.056h.95l3.468-3.473l-.47-.475l-.455-.488l-3.493 3.486zm3.948-3.948l-.455-.488l.925.963z"
                      />
                    </svg>
                  </Link>
                  <button
                    onClick={async () => {
                      await remove(item["_id"]);
                    }}
                    className="btn btn-square btn-ghost"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                    >
                      <path
                        fill="currentColor"
                        d="M7.616 20q-.691 0-1.153-.462T6 18.384V6H5V5h4v-.77h6V5h4v1h-1v12.385q0 .69-.462 1.153T16.384 20zm2.192-3h1V8h-1zm3.384 0h1V8h-1z"
                      />
                    </svg>
                  </button>
                </li>
              </ul>
            </div>
          );
        })}
      </div>
    );
  }
}

export default UserNewsList;
